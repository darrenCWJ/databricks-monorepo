...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--name", required=True, help="apps/<name>/ target — e.g. finance-budget-recon"
    )
    parser.add_argument("--team", required=True, help="owning team, e.g. finance")
    parser.add_argument(
        "--mode",
        choices=("history", "fresh"),
        default="history",
        help="history preserves git log; fresh copies the working tree only",
    )
    parser.add_argument(
        "--allow-dirty", action="store_true", help="skip the hygiene check (NOT recommended)"
    )
    ...
    if args.mode == "history":
            ...
            try:
                ...
                run(
                    [
                        ...
                        "-m",
                        f"Migrate {args.source.name} into apps/{args.name} (history preserved)",
                        f"{remote_name}/main",
                    ]
                )
            finally:
        ...
        run(
            [
                ...
                "commit",
                "-m",
                f"Migrate {args.source.name} into apps/{args.name} (fresh, from {src_sha[:12]})",
            ]
        )
    ...
    print("  2. Add a row to docs/data-architecture.md")
    print("  3. Add CODEOWNERS rule above")
    print(f"  4. Run: just lint apps/{args.name}/ && just test apps/{args.name}/")
    print(f"  5. Run: just bundle-validate apps/{args.name}/")
    print("  6. Open the MR with a change-ticket ID")
    return 0
