...
- `@wei_hao_tan @jeffrey_siew` + `@wei_hao_tan @jeffrey_siew` (if any column is
  classified `Restricted`)
- `@wei_hao_tan @jeffrey_siew` (if you touched `.gitlab-ci.yml` or anything in
  `infra/`)
...
- **Skipped the `meta.*` block on dbt columns.** Pre-commit blocks the
  MR. Fix: classify every column. If unsure, `@wei_hao_tan @jeffrey_siew` will
  advise.
