...
- Sets `meta.no_mask_required: true` with a comment explaining why (needs
  `@wei_hao_tan @jeffrey_siew` review).
...
```sql
-- As @wei_hao_tan @jeffrey_siew (no pii_readers membership):
SELECT email FROM cdo_dev.silver.fct_customers LIMIT 1;
-- email
-- REDACTED

-- As @wei_hao_tan @jeffrey_siew (member of cdo_pii_readers_dev):
-- email
-- jane@example.com
```
...
Open MR. `@wei_hao_tan @jeffrey_siew` + `@wei_hao_tan @jeffrey_siew` approve. Apply.
...
2. UC checks Layer 2: analyst (via @wei_hao_tan @jeffrey_siew) has SELECT on cdo_prod.gold. Yes.
...
`@wei_hao_tan @jeffrey_siew` walks each. Stale entries removed via MR. The folder
