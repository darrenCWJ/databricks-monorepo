...
- `projects/<team>-*/` — your team's deploy units
- `projects/<team>-*/src/` — testable Python (the home of your business logic)
- `projects/<team>-*/tests/` — pytest, unit + integration
- `projects/<team>-*/bundle.yml` — the DAB definition (jobs, schedules, clusters)
- `libs/<team>_common/` — your team's shared code
- `libs/common_*` — platform-wide libs (read-only; changes go through @wei_hao_tan @jeffrey_siew)
...
- `make test P=projects/<that-app>` — confirm tests pass on your laptop
...
1. Never cross `projects/<team-X>` and `projects/<team-Y>` in one MR.
...
4. New deploy unit = new directory under `projects/` with `bundle.yml` + `AGENTS.md`.
